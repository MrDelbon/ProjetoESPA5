...
# Exemplo de teste de Kruskal-Wallis
resultado_teste_kruskal_wallis <- kruskal.test(SE_IS  ~ ANO_IS, data = casos_humanos)
print(resultado_teste_kruskal_wallis)
...
# Filtrando os dados para o município específico
municipio_especifico <- 1

dados_municipio <- inner_join(
  casos_humanos %>% filter(COD_MUN_LPI == municipio_especifico) %>%
    select(DT_IS, NUMERO_CASOS),
  epizootias %>% filter(COD_MUN_OCOR == municipio_especifico) %>%
    select(DATA_OCOR, NUMERO_CASOS),
  by = c("DT_IS" = "DATA_OCOR")
)

# Exibindo os dados do município específico
